...
def serf_event_stream_thread():
    """MODIFIED: Use 'serf stream' for reliable JSON events."""
    while True:
        try:
            logger.info("Launching 'serf stream' to listen for user events...")
            # This command connects to the Serf RPC and streams events as JSON
            cmd_args = [SERF_EXECUTABLE_PATH, "stream", "-format=json", f"-rpc-addr={SERF_RPC_ADDR}", "user"]
            ...
            # This loop reads each line of JSON output
            for line in iter(process.stdout.readline, ''):
                if line.strip():
                    handle_serf_json_event(line)

        except Exception as e:
            logger.critical(f"Serf stream failed: {e}")
            with metrics_lock: app_metrics["serf_monitor_status"] = f"Error: {e}"
        
        logger.warning("Serf stream process exited. Restarting in 5 seconds...")
        time.sleep(5)

def health_check_thread():
    """Periodically checks the status of Serf members and CometBFT."""
    while True:
        try:
...
def handle_serf_json_event(line: str):
    """MODIFIED: Parses a JSON event line from 'serf stream'."""
    try:
        event_data = json.loads(line)
        # We only care about user events, though the stream is already filtered
        if event_data.get("Event") == "user":
            process_serf_user_event(event_data)
    except json.JSONDecodeError:
        logger.warning(f"Could not decode JSON from Serf stream: {line}")

def process_serf_user_event(event_data: dict):
    """Processes a parsed Serf user event and sends it to CometBFT."""
    ...
    new_activity = {"name": event_name, "cometbft_response": "Broadcasting...", "status": "pending"}
    ...
    def cb(response: CometBFTTxResponse, entry=new_activity):
        with metrics_lock:
            ...
            if response.is_successful:
                entry["cometbft_response"] = f"Success (Height: {response.height})"
                entry["status"] = "success"
            else:
                entry["cometbft_response"] = f"Failed (CheckTx:{response.check_tx_code}, DeliverTx:{response.deliver_tx_code})"
                entry["status"] = "failure"
                logger.error(f"CometBFT transaction failed for event '{event_name}': {response.log}")

    try:
        decoded_tx_bytes = base64.b64decode(payload_b64)
        cometbft_client.broadcast_and_commit_tx(decoded_tx_bytes, cb)
    except Exception as e:
        logger.error(f"Failed to decode or broadcast payload: {e}")
        with metrics_lock:
            new_activity["cometbft_response"] = "Broadcast Error"
...
def start_background_threads():
    ...
    with threads_lock:
        if not threads_started:
            # The event stream thread is the new 'monitor'
            threading.Thread(target=serf_event_stream_thread, daemon=True).start()
            threading.Thread(target=health_check_thread, daemon=True).start()
            threads_started = True
            logger.info("Started Serf event stream and health check threads.")
