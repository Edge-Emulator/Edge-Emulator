...
def get_event_signature(event_name: str, payload_b64: str) -> str:
    """Generates a unique signature for a Serf event to detect duplicates."""
                ...
                if line.startswith("Name:"):
                    ...
                    if "name" in current_event_info:
                        ...
                        if current_event_info["payload_full"] != "Parsing Error":
                            ...
                            event_signature = get_event_signature(event_name_to_process, payload_b64_to_process)

                            if event_signature in processed_monitor_events:
                                logger.debug(f"Skipping duplicate event (already processed by monitor): {event_name_to_process}")
                                parsing_event_info_block = False
                                current_event_info = {}
                                continue
                            processed_monitor_events.append(event_signature)
                            ...
                            def broadcast_response_callback(response: MockResponseCheckTx, activity_entry=activity_entry, event_name_for_log=event_name_to_process):
                                with metrics_lock:
