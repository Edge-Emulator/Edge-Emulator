        ...
        try:
            ...
            for line in iter(process.stdout.readline, ''):
                ...
                if parsing_event_info_block:
                    ...
                    if "Received event: user-event:" in line and " Payload: " in line: # Check if " Payload: " substring exists in the line
                        try:
                            # Parse a single-line user event that DOES contain a payload.
                            # Example: "...user-event: event-name Payload: base64payload"
                        ...
                        except Exception as e: # Catch any errors from processing this specific single-line user-event with payload
                            logger.error(f"Error processing single-line user event with payload: {e}. Line: {line}")
                            # Do not 'return' here, let the loop continue for next lines.
                    else: # This handles other [INFO] agent/serf lines that are not user-events with explicit payloads.
