...
class MockResponseCheckTx:
    """
    A class to encapsulate the response structure from CometBFT's broadcast_tx_sync.
    """
...
class CometBFTMempoolClient:
    """
    Client to interact with CometBFT RPC for broadcasting and polling transactions.
    """
    def __init__(self, rpc_url: str):
        self.rpc_url = rpc_url
        logger.info(f"CometBFTMempoolClient initialized with RPC URL: {self.rpc_url}")

    def BroadcastTx(self, tx_bytes: bytes, cb: callable) -> None:
        """
        Broadcasts a transaction to CometBFT's mempool using broadcast_tx_sync.
        The callback `cb` receives the immediate response from CheckTx.
        """
        ...
        try:
            ...
            if "result" in rpc_result:
                ...
                logger.info(f"CometBFT RPC broadcast response: {comet_response.to_dict()}")
                cb(comet_response)
    ...
    def PollTxStatus(self, tx_hash: str, callback: callable, max_attempts: int = 20, interval_sec: int = 1) -> None:
        ...
        def _poll():
            ...
            while attempts < max_attempts:
                attempts += 1
                try:
                    # Use the 'tx' endpoint for exact hash lookup
                    # Note: hash needs '0x' prefix for the RPC call
                    ...
                    if "result" in result and result["result"] is not None and "tx_result" in result["result"]:
                        ...
                        return # Transaction found, stop polling
                    else:
                    ...
                    # If connection lost, might be better to stop polling or extend interval
                    break
                except Exception as e:
        ...
        # Run polling in a separate thread to not block the main serf monitor loop
        polling_thread = threading.Thread(target=_poll, name=f"TxPollThread-{tx_hash[:5]}")
        polling_thread.daemon = True # Allow main app to exit even if polling is ongoing
        polling_thread.start()

    # Stub methods (as they were, not directly used in the current flow)
    ...
    while True:
        ...
        if current_time - last_cometbft_status_check_time > COMETBFT_STATUS_CHECK_INTERVAL:
            try:
                ...
                with metrics_lock:
                    # Only update general status if not currently broadcasting/polling
            ...
            for line in iter(process.stdout.readline, ''):
                ...
                if line.startswith("Name:"):
                    ...
                    if parsing_event_info_block:
                        try:
                            # Extract hex bytes string, remove "0x" and ", "
                            hex_bytes_str = line.split("Payload: []byte{", 1)[1].strip("}").replace("0x", "").replace(", ", "")
                            event_payload_b64_to_process = base64.b64encode(bytes.fromhex(hex_bytes_str)).decode('utf-8')
                        except Exception as e:
                        ...
                        if "name" in current_event_info:
                            ...
                            with metrics_lock:
                                ...
                                # Prepare activity entry with initial status
                                new_activity = {
                            ...
                            try:
                                ...
                                with metrics_lock:
                                    new_activity["cometbft_broadcast_response"] = f"Decode Error: {e}"
                                    new_activity["cometbft_consensus_status"] = f"Decode Error: {e}"
                                parsing_event_info_block = False
                                current_event_info = {}
                                continue

                            def broadcast_response_callback(response: MockResponseCheckTx, activity_entry=new_activity, event_name_for_log=event_name_to_process):
                                """Callback for the initial broadcast_tx_sync response."""
                                with metrics_lock:
                                    activity_entry["cometbft_broadcast_response"] = f"Code: {response.code}, Log: {response.log[:50]}..."
                                    if response.code == 0 and response.hash:
                                        ...
                                        # Start polling for consensus using the returned transaction hash
                                        ...
                                        activity_entry["cometbft_consensus_status"] = f"Broadcast Failed (Code: {response.code}) Log: {response.log[:50]}..."

                            def update_consensus_status(activity_entry, success, tx_data, msg):
                                """Callback for updating activity log with consensus status."""
                                with metrics_lock:
                                    if success:
                                        # Show height and ABCI code/log from the committed transaction result
                                        abci_code = tx_data.get('tx_result', {}).get('code', -1)
                                        abci_log = tx_data.get('tx_result', {}).get('log', '')
                                        activity_entry["cometbft_consensus_status"] = f"Committed! Height: {tx_data.get('height')}, Code: {abci_code}, Log: {abci_log[:50]}"
                                    else:
                                        activity_entry["cometbft_consensus_status"] = msg

                            # Call BroadcastTx
                            mempool_client.BroadcastTx(decoded_tx_bytes, broadcast_response_callback)
                    ...
                    # Fallback for simpler single-line user events without explicit Name/Payload tags
                    if not parsing_event_info_block:
                        try:
                            ...
                            if len(parts) == 2:
                                ...
                                mempool_client.BroadcastTx(decoded_tx_bytes,
                                    lambda response: broadcast_response_callback(response, new_activity, event_name_to_process))
...
def index():
    ...
    if current_metrics["cometbft_rpc_status"] == "Connected":
        ...
        comet_status_color = "bg-yellow-500" # Indicate ongoing activity
