...
START=1
END=50
#TOPO_FILE="50node_update.yml"

# Multiple bridges supported (default list)
BRIDGES=("switch_a" "switch_b" "switch_c" "switch_d" "switch_e")
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --bridge|--bridge-name)
      # Allow multiple --bridge flags, append to BRIDGES
      BRIDGES+=("$2")
      shift 2
      ;;
    ...
    --help|-h)
      echo "Usage: $0 [--prefix PFX] [--start N] [--end M] [--topo FILE] [--bridge NAME (repeatable)]"
      exit 0
      ;;
...
# ------------------ Start ------------------
# Uncomment if you want hard checks for tools:
#require_cmd docker
#require_cmd clab

ensure_openvswitch

# Ensure all requested OVS bridges exist
for br in "${BRIDGES[@]}"; do
  ensure_bridge "$br"
done

#deploy_topology   # still optional / commented, as in your version

# Host-side scripts
run_host_script "$SCRIPT_IP"

# ---------- Step: Create/Update node.json in each container ----------
log "Creating/updating node.json in containers ${PREFIX}{${START}..${END}} ..."
...
for i in $(seq "$START" "$END"); do
  ...
  if ! docker ps --format '{{.Names}}' | grep -Fxq "$container"; then
    warn "Container '$container' not running; skipping node.json."
    continue
  fi

  # Grab eth1 IPv4 address from inside the container
  ip_address="$(
    docker exec -u root "$container" sh -c \
      "ip -4 addr show dev eth1 2>/dev/null | awk '/inet /{print \$2}' | cut -d/ -f1 || true"
  )"

  if [[ -z "$ip_address" ]]; then
    warn "Could not determine eth1 IP for '$container'; skipping node.json."
    continue
  fi

  log "[$container] eth1 IP = $ip_address -> writing ${CONTAINER_APP_DIR}/node.json"

  # Ensure app dir exists
  docker exec -u root "$container" sh -c "mkdir -p '$CONTAINER_APP_DIR'"

  # Create/overwrite node.json
  docker exec -u root -i "$container" sh -c "cat > ${CONTAINER_APP_DIR}/node.json" <<EOF
{
  "node_name": "$container",
  "bind": "0.0.0.0:7946",
