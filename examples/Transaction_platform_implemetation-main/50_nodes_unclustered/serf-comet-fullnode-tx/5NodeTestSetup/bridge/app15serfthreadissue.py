...
def serf_monitor_thread(serf_exec_path: str, rpc_addr: str, mempool_client: CometBFTMempoolClient):
    ...
    # State variables for multi-line event parsing
    event_buffer = {}
    collecting_event_info = False
    ...
    while True:
        ...
        try:
            ...
            for line in iter(process.stdout.readline, ''):
                ...
                # --- REFINED SERF MONITOR OUTPUT PARSING ---
                if line.startswith("Event Info:"):
                    collecting_event_info = True
                    event_buffer = {"type": "Serf User Event"} # Start buffering for a new event
                    continue # Move to next line to parse Name/Payload

                if collecting_event_info:
                    if line.startswith("Name:"):
                        event_buffer["name"] = line.split("Name:", 1)[1].strip().strip('"')
                    elif line.startswith("Payload:"):
                        try:
                            hex_bytes_str = line.split("Payload: []byte{", 1)[1].strip("}").replace("0x", "").replace(", ", "")
                            payload_bytes = bytes.fromhex(hex_bytes_str)
                            event_buffer["payload_full"] = payload_bytes.decode('utf-8') # This is the clean base64 string
                        except Exception as e:
                            logger.error(f"Error parsing hex bytes from Serf monitor payload: {e}. Line: {line}")
                            current_event_info["payload_full"] = "Parsing Error" # Indicate parsing failure
                            event_buffer = {} # Discard this potentially corrupted event block
                            collecting_event_info = False # Reset state machine
                            continue # Move to next line

                    # If we have both name and payload, process the event
                    if "name" in event_buffer and "payload_full" in event_buffer:
                        # Event is fully collected, process it
                        event_name_to_process = event_buffer["name"]
                        payload_b64_to_process = event_buffer["payload_full"] # This is the clean base64 string

                        # Determine the transaction hash from the Serf payload (expected to be {"tx_hash": "..."})
                        ...
                        if event_name_to_process.startswith("report-tx-status-"):
                            ...
                            collecting_event_info = False
                            continue # Don't process as regular tx if it's a report
                        ...
                        # --- Prepare kvstore-compatible transaction for CometBFT ---
                        ...
                        event_buffer = {} # Clear buffer after successful processing
                        collecting_event_info = False # Reset state machine
                    else: # This handles other lines within the Event Info block that are not Name/Payload (e.g., Coalesce, Event, LTime)
                         # We don't reset collecting_event_info because we're still collecting the event.
                        pass # Do nothing, continue collecting the block.

                # --- Handle generic single-line Serf agent logs ---
                # This catches any other log lines from Serf monitor that are NOT part of a multi-line Event Info block.
                # These are usually informational messages or internal Serf events that don't need app-level processing.
                else:
