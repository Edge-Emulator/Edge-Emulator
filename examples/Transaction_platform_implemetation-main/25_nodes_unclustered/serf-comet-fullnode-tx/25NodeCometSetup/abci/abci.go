...
func NewMyApp(db *pebble.DB) *MyApp {
	app := &MyApp{
		state: &State{
			DB:        db,
...
func (app *MyApp) updateValidator(vReqTx string) {
	...
	for _, val := range vtx.Validator {
		pubkey, err := cryptoenc.PubKeyFromTypeAndBytes(val.PubKeyType, val.PubKeyBytes)
		if err != nil {
		...
		case AddValidatorType, UpdateValidatorType:
			...
			app.addOrUpdateStateValidator(val, addr)
			app.appendValidatorUpdateOnce(addr, val)
